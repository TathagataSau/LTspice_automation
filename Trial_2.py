# -*- coding: utf-8 -*-
"""
Created on Thu Jun 15 23:56:11 2023
...
def chplot(n1,n2):
    ...
    netlist_path = r"C:\Users\Tathagata Sau\OneDrive\Documents\LTspiceXVII\tmp5.cir"
    subprocess.call([ltspice_exe_path, '-Run', netlist_path])
    path = r"C:\Users\Tathagata Sau\OneDrive\Documents\LTspiceXVII"
    filename = "tmp5"
    path += '\\tmp5.raw'
...
def toxxl(current,tox,filename,path,n):
    ...
    if('tox_n_varition_Final.xlsx' in dir_list):
        ...
        location = "E:/PROECT_Main/LTspiceXVII/LTspiceXVII/"
...
def vthxl(current,vtharray,filename,path,n):
    ...
    path1 = "C://Users//Tathagata Sau//OneDrive//Documents//LTspiceXVII"
    dir_list = os.listdir(path1)
    if('VTH_n_varition_Final.xlsx' in dir_list):
        print("Duplicate Found")
        file = 'VTH_n_varition_Final.xlsx'
        location = "C:/Users/Tathagata Sau/OneDrive/Documents/LTspiceXVII/"
        path1 = os.path.join(location, file)
        os.remove(path1)
    os.chdir(path)
    filename+=".xlsx"
...
def vthandtoxxl(current,vtharray,toxvalues,filename,path,n):
    ...
    path1 = "C://Users//Tathagata Sau//OneDrive//Documents//LTspiceXVII"
    dir_list = os.listdir(path1)
    if('VTH_P_varition_Final.xlsx' in dir_list):
        print("Duplicate Found")
        file = 'VTH_P_varition_Final.xlsx'
        location = "C:/Users/Tathagata Sau/OneDrive/Documents/LTspiceXVII/"
        path1 = os.path.join(location, file)
        os.remove(path1)
    os.chdir(path)
...
if (n==1):
    ...
    df = pd.DataFrame(pd.read_excel(r'E:\PROECT_Main\LTspiceXVII\LTspiceXVII\tox_n_varition_Final.xlsx'))
    custom_SVM(df)
if(n==2):
    sigma=float(input('Please enter the sigma value in percentage\n(for ex:for 3%,5%,7%... variation of vth just enter 3,5,7,..respectively )\n'))
    txtfile=input('Please enter the file in proper format including extension(.cir or .txt)\n')
    ...
    df = pd.DataFrame(pd.read_excel(r'E:\PROECT_Main\LTspiceXVII\LTspiceXVII\VTH_n_varition_Final.xlsx'))
...
if(n==4):
    ...
    n2=int(input('Please enter the choice of y index you want to plot:'))
    res = chplot(n1,n2) 
    df = pd.DataFrame(pd.read_excel(r'E:\PROECT_Main\LTspiceXVII\LTspiceXVII\VTH_P_varition_Final.xlsx'))
    m,k,s =m.cal(res) 
    print("For Real Values : ")
    print("Mean value is: ",m)
    print("kurtosis value is: ",k)
